...
def solve_by_rotation(n):
	...
	GENERAL_DOMAIN_DICT = {0: {0}, 1: {0, 1, 2, 3}, 3: {0, 1, 2, 3}, 5: {0, 1}, 7: {0, 1, 2, 3}, 15: {0}}
	for i, tile_number in enumerate(new_grid):
		domain[i] = GENERAL_DOMAIN_DICT[tile_number]


	# First : Contrainte unaire : work on N (addConstraint is only for Binary constraint)
	TOP_SIDE_CONSTRAINT_DICT = {0: {0}, 1: {1,2,3}, 3: {1,2}, 5: {1}, 7: {1}}
	LEFT_SIDE_CONSTRAINT_DICT = {0: {0}, 1: {0,2,3}, 3: {2,3}, 5: {0}, 7: {2}}
	RIGHT_SIDE_CONSTRAINT_DICT = {0: {0}, 1: {0,1,2}, 3: {0,1}, 5: {0}, 7: {0}}
	BOTTOM_SIDE_CONSTRAINT_DICT = {0: {0}, 1: {0,1,3}, 3: {0,3}, 5: {1}, 7: {3}}
	...
	RIGHT_SIDE_CONNECTOR_PRESENT_BINARY_CONSTRAINT_DICT = {0: {}, 1: {3}, 3: {2,3}, 5: {1}, 7: {1,2,3}, 15: {0}}
	...
	BOTTOM_SIDE_CONNECTOR_ABSENT_BINARY_CONSTRAINT_DICT = {0: {0}, 1: {0, 1, 3}, 3: {0,3}, 5: {1}, 7: {3}, 15: {}}
	TOP_SIDE_CONNECTOR_ABSENT_BINARY_CONSTRAINT_DICT = {0: {0}, 1: {1, 2, 3}, 3: {1, 2}, 5: {1}, 7: {1}, 15: {}}
	...
	for x in range(n**2 - 1):
		...
		if x < n**2 - n:
			...
			tile_bottom_presence_set = BOTTOM_SIDE_CONNECTOR_PRESENT_BINARY_CONSTRAINT_DICT[new_grid[x]]
			tile_top_presence_set = TOP_SIDE_CONNECTOR_PRESENT_BINARY_CONSTRAINT_DICT[new_grid[x+n]]
			bottom_side_presence_constraint = {(u, v) for u in tile_bottom_presence_set for v in tile_top_presence_set}

			# Absence of a connecter
			tile_bottom_absence_set = BOTTOM_SIDE_CONNECTOR_ABSENT_BINARY_CONSTRAINT_DICT[new_grid[x]]
			tile_top_absence_set = TOP_SIDE_CONNECTOR_ABSENT_BINARY_CONSTRAINT_DICT[new_grid[x+n]]
			bottom_side_absence_constraint = {(u, v) for u in tile_bottom_absence_set for v in tile_top_absence_set}

			bottom_side_constraint = bottom_side_presence_constraint | bottom_side_absence_constraint
